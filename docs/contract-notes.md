...
| 分页恒五键 | PageResultOut |
| 统计计数 int 出参 | stats 3 action 全 int，avg int |
...
| TASK_START 落库后 fire（C12） | PersistTasksAsync.NotifyTaskStartAsync |
...
| updateDefine 顶层 JSON 兼容 | ContentBytes |
| designRedeploy 继承原 version（C29） | DesignRedeployAsync |
