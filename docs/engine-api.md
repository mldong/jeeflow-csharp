...
| `ExecuteAndJumpTaskAsync(taskId, operator, args, nodeName)` | 跳转（JUMP=4）/ 退回上一步（ROLLBACK=3，nodeName 传空）。**本栈现实现为拓扑版**：沿首入边退回、新 todo actor=退回操作人；契约已统一到**血缘版**（上一步取 `task_parent_id` 行、参与者=该行办结人，见规范「引擎操作 04 · 退回上一步」），本栈待对齐（issues/121） |
