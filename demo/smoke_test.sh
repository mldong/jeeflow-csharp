...
PASS=0; FAIL=0
# 提取纯 grep/sed：托管机可能无 python（run 34009961550 实证），Windows 下 python3 是
# Store 假 stub 会静默失败——不引入任何解释器依赖（id 出口恒字符串化数字，正则可安全抓取）
...
# 首个 "key":"数字串" 的数字（用于 processInstanceId 等字符串化 id）
str_id()  { echo "$1" | grep -oE "\"$2\":\"[0-9]+\"" | head -1 | grep -oE '[0-9]+'; }
# 首个 "key":整数值（用于 state 等数值字段）
num_val() { echo "$1" | grep -oE "\"$2\":[0-9]+" | head -1 | grep -oE '[0-9]+'; }
...
IID=$(str_id "$START" processInstanceId)
